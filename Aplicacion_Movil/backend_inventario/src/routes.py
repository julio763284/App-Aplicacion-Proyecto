...
def init_routes(app):
    ...
    def login():
        ...
        if user:
            ...
            if rol_usuario in ('superadmin', 'admin', 'cliente'):
                return jsonify({
                    "status": "success",
    ...
    @app.route('/imagen_producto', methods=['GET'])
    def imagen_producto():
        ruta = request.args.get('ruta', '')
        if not ruta or '..' in ruta:
            return jsonify({"error": "Ruta inválida"}), 400

        base = r'C:\Users\joinner.fernandez\Desktop\pag def\MiTiendaWed'
        ruta_completa = os.path.join(base, ruta.lstrip('/').replace('/', os.sep))
    ...
    def verificar_stock():
        if verificar_stock_bajo():
            return jsonify({"status": "success", "message": "Verificación realizada"}), 200
        return jsonify({"status": "error", "message": "No se pudo verificar"}), 500
